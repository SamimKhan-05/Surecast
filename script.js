...
window.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('load', () => setTimeout(() => fixMapSize(), 180), { once: true });
...
function fixMapSize(attempt = 0){
  ...
  if (!hasSize && attempt < 6) {
    setTimeout(() => fixMapSize(attempt + 1), 140);
    return;
  }

  if (attempt === 0) {
    requestAnimationFrame(() => {
      try { map.invalidateSize(false); } catch (error) {}
    });
    setTimeout(() => {
      try { map.invalidateSize(false); } catch (error) {}
    }, 180);
